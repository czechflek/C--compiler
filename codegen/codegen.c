...
int handle_id(ast_node * node) {
    ...
    if (num_args == 0) {
        ...
        if (var.offset < 0) {
            whops("error: variable undeclared (first use in this function)", info->line_no);
        }
        if (var.size == 1)
        ...
        if (args[0]->symbol->grammar_symbol == EXPR_LIST) {
		    ...
		    if (var.offset < 0) {
		        whops("error: array undeclared (first use in this function)", info->line_no);
		    }
...
void handle_fun_decl(ast_node * node) {
    ...
    if (dummy.name != NULL) whops("error: function already defined.", node->symbol->line_no);
...
int handle_assign(ast_node * node) {
    ...
    info = args[0]->symbol;
    if (info->token != ID) whops("error: incompatible lvalue.", info->line_no);
...
int handle_read(ast_node * node) {
    ...
    ast_node ** args = get_childlist(node); // a  = b = c
    info = args[0]->symbol;
    if (info->token != ID) whops("error: incompatible rvalue.", info->line_no);
...
int handle_expr_list(ast_node * node, FunDef fun) {
    ...
    if (num_args < fun.param_count) whops("error: too few arguments to function", node->symbol->line_no);
    if (num_args > fun.param_count) whops("error: too many arguments to function", node->symbol->line_no);
    ...
    for (i = 0; i < fun.param_count; i++) {
        ...
        if (var.offset >= 0) {
            if (var.size != (fun.param_type[i] == T_INT ? 4 : 1)) {
                whops("error: non-matching argument types", node->symbol->line_no);
            }
        }
        
        if (is_array) {
        	if (var.offset < 0) {
		        whops("error: array undeclared (first use in this function)", node->symbol->line_no);
		    }
        } else {
...
int call_function(ast_node * node) {
    ...
    if (fun.name == NULL) whops("error: function not declared.", node->symbol->line_no);
    //backup_params(&fun);
